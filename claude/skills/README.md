...
スキルは `.claude/skills/` ディレクトリに格納されたAIの拡張機能です。各スキルには `SKILL.md` が含まれ、Claude Code が自動的に読み込みます。
...
スキルは Claude Code 標準の配置先である `.claude/skills/` に直接置いています。リンクやジャンクションは使いません。Claude Code はこのディレクトリを自動で読み込むため、追加設定は不要です。

スキルを追加するときは `.claude/skills/` に新しいフォルダを作り、`SKILL.md` を置くだけで利用できます。
