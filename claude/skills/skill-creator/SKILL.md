...
# 必要に応じてサブディレクトリ作成
New-Item -ItemType Directory -Force "[WORKSPACE]/.claude/skills/<skill-name>/scripts"
New-Item -ItemType Directory -Force "[WORKSPACE]/.claude/skills/<skill-name>/references"
New-Item -ItemType Directory -Force "[WORKSPACE]/.claude/skills/<skill-name>/assets"
```
...
スキルは Claude Code 標準の `.claude/skills/` に置く。フォルダを作って `SKILL.md` を置けば自動認識される（リンクや追加登録は不要）。

### Step 8: README更新

`[WORKSPACE]/.claude/skills/README.md` があれば、スキル一覧に追加。
...
```powershell
# 対象スキルを読む
Get-Content "[WORKSPACE]/.claude/skills/<skill-name>/SKILL.md"

# 行数を確認
(Get-Content "[WORKSPACE]/.claude/skills/<skill-name>/SKILL.md").Count
```
...
- `[WORKSPACE]/.claude/skills/README.md` - スキル一覧と作成ガイド
